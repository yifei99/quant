...
from factors.factor_definitions import LiquidationWithMAFactor
...
def main():
    ...
    logger.info("Starting Liquidation-MA factor mining...")
    ...
    test_configs = {
        ...
        'factors': [
            {
                'class': LiquidationWithMAFactor,
                'name': 'liquidation_ma_reversion',
                'params': {
                    'threshold': np.arange(0, 0.016, 0.000005).tolist(),  
                    'ma_period': list(range(7, 1000)) 
                }
            }
        ]
    }
        ...
        for logic_config in test_configs['trading_logics']:
            for factor_config in test_configs['factors']:
                try:
                    ...
                    liquidation_data['Date'] = pd.to_datetime(liquidation_data['hour']).dt.tz_localize(None)
                    ...
                    amounts_data['Date'] = pd.to_datetime(amounts_data['hour']).dt.tz_localize(None) + pd.Timedelta(hours=1)
